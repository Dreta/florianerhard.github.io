...
| A | | $$s_1$$ | ... | $$s_{i-1}$$ | $$s_{i}$$ | ... |  $$s_n$$ |
| |
| $$t_1$$ |
| ... |
| $$t_{j-1}$$ ||||$$A_{i-1,j-1}$$|$$A_{i,j-1}$$
| $$t_j$$ |||| $$A_{i-1,j}$$ | $$A_{i,j}$$
| ... |
| $$t_m$$ |
...
The type of alignment found by the Needleman-Wunsch algorithm is call a global alignment: Every character that is not aligned is penalized. For many applications, this is too restrictive:
...
If the trailing gap is not penalized, the alignment score of $$s$$ and $$t$$ is equal to the alignment score of $$s$$ and $$t^j$$. Of course, this score is available in $$A_{j,m}$$. Thus, we only have to find the maximal score in the last column of $$A$$ to find the optimal alignment not penalizing a trailing gap in $$s$$. Similarly, we have to find the maximal score in the bottom row for optimal alignments not penalizing a trailing gap in $$t$$, or, the maximal value on both the last row and last column for the optimal alignment not penalizing a trailing gap in either $$s$$ or $$t$$.
...
An alignment of this form also is a prefix of the full alignment that must not be penalized. Thus, the maximum must be computed as 
